...
for (let btn of blogDisplayBtnList) {
    ...
    dispBtn.addEventListener("click", function () {
        postDisplayCount = increasePostDisplayLimit(
            postDisplayCount,
            3,
            postContent.length
        );
...
function getUuidFromBlogPostEl(targetEl) {
    ...
    if (
        targetEl.parentNode.nodeName === "LI" ||
        targetEl.parentNode.id === "hero-post"
    ) {
...
function renderBlogPostPage(postId) {
    ...
    postContainerEl.appendChild(
        createPostDateEl("h3", selectedPost, fullDateOptions)
    );
